...
const handler: ExportedHandler<Env> = {
  fetch: (
    ...
    const slackApp = new SlackApp<Env>({ env })
      ...
      .event(
        greet.start.event.reaction.added.name,
        greet.start.event.reaction.added.handler
      )
      .event(
        greet.start.event.reaction.removed.name,
        greet.start.event.reaction.removed.handler
      );
